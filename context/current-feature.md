...
- Forgot Password
  - [x] `src/app/api/auth/forgot-password/route.ts` — POST: looks up user, generates 32-byte reset token stored as `reset:<email>` in VerificationToken, sends reset email via Resend; always returns `{ success: true }` to prevent email enumeration
  - [x] `src/app/api/auth/reset-password/route.ts` — POST: validates token, `startsWith('reset:')` guard prevents verification tokens being used as reset tokens, bcrypt 12 rounds, deletes token on success
  - [x] `src/app/(auth)/forgot-password/page.tsx` — server component; imports `ForgotPasswordForm`
  - [x] `src/components/auth/ForgotPasswordForm.tsx` — email input; shows in-place success message on send, no redirect
  - [x] `src/app/(auth)/reset-password/page.tsx` — async server component; reads `?token=` from searchParams, passes as prop to `ResetPasswordForm`
  - [x] `src/components/auth/ResetPasswordForm.tsx` — password + confirm form; shows "Password Reset!" success screen with sign-in link on completion
  - [x] `src/lib/email.ts` — `sendPasswordResetEmail(to, token, baseUrl)` added; shared `emailTemplate()` helper refactored out to eliminate duplication with verification email
  - [x] `src/components/auth/SignInForm.tsx` — "Forgot password?" link added below password field
  - [x] All auth pages (`sign-in`, `register`, `forgot-password`, `reset-password`) refactored to server components; client form logic extracted to `src/components/auth/`
  - [x] `src/hooks/useForm.ts` — custom hook extracting shared `loading`/`error`/`handleSubmit` pattern from all four auth form components
  - [x] `CLAUDE.md` — added rule: page.tsx files must always be server components
