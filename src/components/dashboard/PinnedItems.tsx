...
export default function PinnedItems({ pinnedCollections, pinnedItems }: Props) {
    ...
    <section aria-labelledby='pinned-heading'>
      <div className='flex items-center justify-between mb-3'>
        <div className='flex items-center gap-2'>
          ...
          <h2 id='pinned-heading' className='text-sm font-semibold uppercase tracking-wider text-muted-foreground'>Pinned</h2>
        </div>
            ...
            <CarouselItem key={`${card.kind}-${card.data.id}`} className='pl-3 sm:basis-1/2 lg:basis-1/3'>
              <div className={cn('h-full rounded-lg transition-all duration-200', i === current ? 'ring-1 ring-foreground/20' : '')}>
                {card.kind === 'collection'
                  ? <CollectionCarouselCard col={card.data} showPin={false} />
                  : <ItemCarouselCard item={card.data} showPin={false} />}
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
